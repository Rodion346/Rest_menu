...
from src.repositories.admin import (
    check_and_del_dish,
    check_and_del_menu,
...
async def read_excel_to_data() -> dict[str, list]:
    """
...
def get_manual_id_from_data(item, data_type):
    if data_type == 'menus':
        ...
        return f'{item[0]}:{item[1]}'  # noqa: E231
    elif data_type == 'dishes':
        return f'{item[0]}:{item[1]}:{item[2]}'  # noqa: E231
    else:
        return None
...
async def process_crud(compared_data: dict[str, list]):
    ...
    for submenu in compared_data['submenus'][0]:
        manual_id = f'{submenu[0]}:{submenu[1]}'  # noqa: E231
        async with semaphore:
    ...
    for dish in compared_data['dishes'][0]:
        manual_id = f'{dish[0]}:{dish[1]}:{dish[2]}'  # noqa: E231
        async with semaphore:
...
async def main_async():
    ...
    new_data = await read_excel_to_data()
