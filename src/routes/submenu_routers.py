...
def create_submenu(menu_id: UUID, submenu: SubmenuIn, submenus_service: SubmenusService = Depends(get_submenus_service)):
    submenu_data = submenus_service.create(schemas=submenu, menu_id=str(menu_id))
    return submenu_data
...
def update_submenu(submenu_id: UUID, submenu_update: dict, submenus_service: SubmenusService = Depends(get_submenus_service)):
    new_submenu = submenus_service.update(id=str(submenu_id), schemas=submenu_update)
    if not new_submenu:
