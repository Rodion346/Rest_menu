...
async def create_update_dish(dish, manual_id: str):
    async for session in get_async_session():
        result = await session.execute(select(Dishes).filter(Dishes.manual_id == manual_id))
        ...
        if db_dish:
            ...
            new_dish = Dishes(title=dish[3], description=dish[4], price=round(dish[5], 2),
